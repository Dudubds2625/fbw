...
export default function HomeScreen({ onStartGame }: HomeScreenProps) {
  ...
  const handleCreateRoom = async () => { const code = generateRoomCode(); const { data, error } = await supabase.from('rooms').insert({ code, host_id: userId, status: 'waiting' }).select().single(); if (error || !data) return Alert.alert('Erro', 'Não foi possível criar a sala. Verifique a conexão.'); await supabase.from('room_participants').insert({ room_code: code, user_id: userId, user_email: userEmail, username }); setCurrentRoom(data); setParticipants([{ id: 'local', room_code: code, user_id: userId, user_email: userEmail, username, is_ready: false, current_hp: 10, max_hp: 10 }]); subscribeToRoom(code); setLobbyModalVisible(true); };
